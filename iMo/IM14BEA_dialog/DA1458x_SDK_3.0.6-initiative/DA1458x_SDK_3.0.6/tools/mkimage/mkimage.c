...
static void usage(const char* my_name)
{
	fprintf(stderr,
		...
		"\n"
		"Usage case #2:\n"
		"  %s multi spi|eeprom [bloader] in_img1 off1 in_img2 off2 off3 out_file\n"
		"\n"
...
static int create_multi_image(int argc, const char* argv[])
{
	...
	unsigned options = 0;
	int oflags, arg_base = 3, res = EXIT_FAILURE;
	int outf = -1, bloader = -1, img1 = -1, img2 = -1;
	unsigned off1, off2, off3;
	off_t offset;
	...
	/* determine if a bootloader image is given */
	if (9 == argc)
		options &= ~BOOTLOADER;
	else if (10 == argc) {
		options |= BOOTLOADER;
	...
	oflags |= O_BINARY;
#endif
	outf = open(argv[arg_base + 5], oflags, S_IRUSR | S_IWUSR);
	if (-1 == outf) {
