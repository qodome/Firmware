...
/*      
      ...
      if (last_tick_init_done == 1) {
          ...
          uint32 current_tick = halSleepReadTimer();
          if (current_tick < last_wakeup_tick) {
              current_tick += 0x01000000;
          }              
          current_tick -= last_wakeup_tick;
          s.power_on_ticks += current_tick;
          if (current_tick > 3000) {
              s.power_on_ticks_max_count++;
          }
          if (current_tick > s.power_on_ticks_max) {
              s.power_on_ticks_max = current_tick;
          }
      }
      //P0_2 = 1;
#endif
