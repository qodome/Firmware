...
// Battery read should update the latest battery level
static void on_rw_authorize_request(ble_bas_t * p_bas, ble_gatts_evt_t * p_gatts_evt)
{
    ...
    if (p_auth_req->type == BLE_GATTS_AUTHORIZE_TYPE_READ) {
        if (p_auth_req->request.read.handle == p_bas->battery_level_handles.value_handle) {
        	battery_request_measure();

        	auth_params.type = BLE_GATTS_AUTHORIZE_TYPE_READ;
        	auth_params.params.read.p_data = NULL;
        	auth_params.params.read.len    = 0;
        	auth_params.params.read.update = 0;

        	APP_ERROR_CHECK(sd_ble_gatts_rw_authorize_reply(p_bas->conn_handle, p_auth_params));
        }
    }
}
