...
void temp_state_init(void)
{
    ...
    memset((uint8 *)&(od_dlist[0]), 0, TEMP_HIST_DEPTH * sizeof(struct temp_hist));
    for (idx = 0; idx < TEMP_HIST_DEPTH; idx++) {
        od_ptr[idx] = &(od_dlist[idx]);
    }
    memset((uint8 *)&od_max, 0, sizeof(od_max));
    ...
    memset((uint8 *)&od_min, 0, sizeof(od_min));
    ...
    memset(&(od_events[0]), 0, TEMP_HEURISTIC_CNT);
}
